...
map<string,string> Coarse2FineFlowWrapper(double * vx, double * vy, double * warpI2,
  ...
  int colType = 0;  //0 or default:RGB, 1:GRAY (but pass gray image with shape (h,w,1))
  ...
  // Used to hold time performance of Optical Flow Phases
  map<string,string> TIMING_PROFILE;

  // call optical flow backend
  OpticalFlow::Coarse2FineFlow( &TIMING_PROFILE, vxFormatted, vyFormatted, warpI2Formatted,
