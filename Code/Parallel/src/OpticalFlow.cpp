...
{
	double smoothFlowStart = omp_get_wtime();
	...
	for(int count=0;count<nOuterFPIterations;count++)
	{
		...
		{
			double beforeLoopStart=omp_get_wtime();
		  ...
		  Laplacian(foo1,u,Phi_1st);
			...
			double beforeLoopEnd=omp_get_wtime();
			//cout<<"["<<count<<"."<<hh<<"] Before Loop: "<<getGradientEnd-getGradientStart<<endl;
			double loopStart=omp_get_wtime();
	...
	double smoothFlowEnd = omp_get_wtime();
...
void OpticalFlow::Coarse2FineFlow(map<string,string>* TIMING_PROFILE, DImage &vx, DImage &vy, DImage &warpI2,const DImage &Im1, const DImage &Im2, int pyramidLevels, int nCores)
{
  ...
  int nCGIterations = 30;
	...
	double PYRAMID_CONSTRUCTION_BEGIN=omp_get_wtime();
	...
	double PYRAMID_CONSTRUCTION_END=omp_get_wtime();
	...
	for(int k=GPyramid1.nlevels()-1;k>=0;k--)
	{
		double PYRAMID_LEVEL_BEGIN=omp_get_wtime();
		double IMAGE_SETUP_BEGIN=omp_get_wtime();
		if(IsDisplay)
			cout << "P["<<k<<"] ";
	...
	// === Output: warp image 2
	double IMAGE_WARP_BEGIN=omp_get_wtime();
	Im2.warpImageBicubicRef(Im1,warpI2,vx,vy);
	warpI2.threshold();
	double IMAGE_WARP_END=omp_get_wtime();
