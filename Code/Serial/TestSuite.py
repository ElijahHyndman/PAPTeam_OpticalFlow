...
# === Test Run Function
def TestRun(Progression='Default',ImagesPerCollection=3,pyramidLevels=(6,),Repetitions=1,outputSuffix=''):
    ...
    print('\t',ImagesPerCollection,'Images each:',pyramidLevels,'Level Pyramids: Repeated',Repetitions,'\n')

    ImagePairGenerator=TestImagePairGenerator()


    # Total Progress bar: Calculate the total number of images that we will work with
    totalImages=len(pyramidLevels)*Repetitions
    ...
    for iteration in range(Repetitions):
        ...
        for COLLECTION in CollectionProgressions[Progression]:
            ...
            for PYRAMIDHEIGHT in pyramidLevels:
                print('+'*20,'Pyramid:',str(PYRAMIDHEIGHT),'+'*20)
                ...
                for IMAGEPAIR in ImagePairs[:ImagesPerCollection]:
                    ...
                    # === Perform calculation
                    CalculateOpticalFlow(IMAGEPAIR,PYRAMIDHEIGHT,outputSuffix)
...
TestRun(Progression='Full',ImagesPerCollection=1,pyramidLevels=(1,2,4,8,16),Repetitions=1,outputSuffix='_serial')
